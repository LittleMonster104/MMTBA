    ...
    for file in files:
        ...
        if "_finish.json" in file:
            ...
            with open(jsonPath, encoding='utf-8') as f:
                ...
                #以上为原版，无法实现动态获取attribute的数量

                #sjy添加
                attribute_num=len(viaJson['attribute'])
                attributeNums=[0]
                if attribute_num>1:
                   temp=0
                   for num in range(attribute_num-1):
                       temp_attributeNum=len(viaJson['attribute'][str(num+1)]['options'])+temp
                       attributeNums.append(temp_attributeNum)
                       temp=temp_attributeNum
                files = {}
                    ...
                    for action in imagen_x['av']:
                        ...
                        if avs != '':
                            ...
                            for av in avArr:
                                ...
                                dicts.append(dict)
                    index = index + 1
with open('./val_without_personID.csv',"w") as csvfile: 
