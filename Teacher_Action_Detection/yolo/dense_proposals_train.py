...
#这部分代码的作用是将人的信息（坐标和概率）提取出来，保存为pkl文件，保存在mywork/dense_proposals_train.pkl中
...
dicts = []
#以下代码为sjy添加，用于计算视频个数
labels_files=os.listdir(labelPath)
labels_files.sort(key=lambda arr: (int(arr[:-7]), int(arr[5:-4])))
video_nums=int(labels_files[-1].split("_")[0])
#以上代码为sjy添加，用于计算视频个数
print(labelPath,avaMin_dense_proposals_path)
for root, dirs, files in os.walk(labelPath):
    #需要修改
    # 原版，达不到效果
    # lenFile = len(files)/3
    #sjy添加
    lenFile = (len(files) / video_nums)-2
    if root == labelPath:
        #排序，防止10排在1的前面
        files.sort(key=lambda arr: (int(arr[:-7]), int(arr[5:-4])))
        for name in files:
